...
<script>
const allNewsData = {{ site.data.news | jsonify }};
const perPage = 10;

// Only include Biodynamics Lab PIs
const BIODYNAMICS_PIS = ["Jeff Hasty", "Lev Tsimring", "Robert Cooper"];

// Filter to only articles tagged with one of the Biodynamics PIs
const newsData = allNewsData.filter(item =>
  item.tags && item.tags.some(tag => BIODYNAMICS_PIS.includes(tag))
);
  ...
  allNews.forEach(item => {
    if (item.tags) {
      item.tags.forEach(tag => {
        if (!BIODYNAMICS_PIS.includes(tag)) {
          tags.add(tag);
        }
      });
    }
